...
if (subscribeForm) {
    subscribeForm.addEventListener('submit', function(e) {
        // لا تمنع السلوك الافتراضي للنموذج لأننا نستخدم الآن معالجة PHP
        
        // الحصول على البريد الإلكتروني من النموذج بصورة صحيحة
        const emailField = this.querySelector('input[type="email"]');
        
        if (emailField && !validateEmail(emailField.value)) {
            e.preventDefault(); // منع الإرسال فقط إذا كان البريد الإلكتروني غير صالح
...
// for categories buttons in blogs page 
        ...
        document.addEventListener("DOMContentLoaded", function () {
            requestAnimationFrame(() => {
                document.querySelector(".top-toolbar").classList.add("loaded");
            });
        });
